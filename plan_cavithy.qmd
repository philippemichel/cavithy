...
Il s'agit d'une étude interventionnelle multi-centrique. Le risque $\alpha$ retenu est de 0,05 & la puissance de 0,8
...
Toutes les analyses seront réalisées selon une procédure «\gls{itt}» , puis secondairement selon une procédure en «\gls{itt} modifiée» (analyse en \gls{itt} ne prenant en compte que les patients ayant été opérés). Une analyse de tolérance en per-protocole sera aussi réalisée.
...
Le décompte des données manquantes sera réalisé & présenté par un tableau voire un graphique. Les dossiers où une des données nécessaires pour l'étude du critère principal (Dosage de la calcémie à J15, présence de signes cliniques d'hypocalcémie à J15\dots) serait manquante seront exclus de l'étude. De même les variables comportant trop de données manquantes ou non utilisables ne seront pas prises en compte après accord du promoteur.

Passé cette étape, les critères de jugements manquants seront imputés à
l'aide d'une méthode d'\gls{mice}, celle des données continues sera effectuée selon la méthode *predictive mean matching*. Les résultats obtenus sur chaque jeu imputé seront ensuite combinés à l'aide de la règle de Rubin (package `missMDA`[@miss]).
...
Un lien entre les données démographiques ou pré-opératoires & la présence d'une hypocalcémie clinique ou biologique sera recherché. Une analyse multivariée sera  réalisée (\gls{mixte} -- package `lm4`[@lme4]) en incorporant toutes les variables ayant une p-value inférieure à  \num{0,20}. Pour cette détermination du meilleur modèle de régression logistique on utilisera les données après imputation des données manquantes (méthode \gls{mice} -- package `missMDA`[@miss]). 

L’analyse principale sera réalisée sur tous les patients inclus. Une analyse complémentaire en population \gls{itt} modifiée (uniquement les patients ayant été opérés) sera réalisée avec le même protocole. 
...
Pour chacun de ces groupe à risque, on décrira la fréquence des d'hypocalcémie pour les patients supplémentés & non supplémentés. Un test statistique pour chaque facteur ne sera réalisé que si le nombre de patients est suffisant.

## Critère 7
*Évaluation de la prévalence des hypocalcémies définitives*

L'hypocalcémie définitive sera appréciée sur le dosage de la calcémie à M3. La différence de fréquence entre les deux groupes sera présentée par un Odd-Ratio avec un test du $\chi_2$ si les effectifs seront suffisants en particulier le nombre d'hypocalcémie par groupe.
