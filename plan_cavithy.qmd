---
subtitle: "Plan d'analyse statistique - V3.0"
cache: true
lof: false
lot: false
---
...
Un lien entre les données démographiques ou pré-opératoires & la présence d'une hypocalcémie clinique ou biologique sera recherché. Une analyse multivariée sera  réalisée (modèle mixte -- package `lm4`[@lme4]en incorporant toutes les variables ayant une p-value inférieure à  \num{0,20}. Pour cette détermination du meilleur modèle de régression logistique on utilisera les données après imputation des données manquantes. 

L’analyse principale sera réalisée sur tous les patients inclus. Une analyse complémentaire en population \gls{itt} modifiée (uniquement les patients ayant été opérés) sera réalisée avev le même protocole. 
...
La différence de fréquence d'une hypocalcémie sévère telle que définie ici sera présentée par un Odd-Ratio avec un test du $\chi_2$. Une analyse en modèle mixte sera présentée.
